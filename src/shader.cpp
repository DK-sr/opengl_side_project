...
{
    if (uniform_lcoation_cache_.find(name) != uniform_lcoation_cache_.end()) {
        return uniform_lcoation_cache_[name];
    }
    GLCall(int location = glGetUniformLocation(renderer_id_, name.c_str()));
    if (location == -1)
    {
        std::cout << "Warning: uniform '" << name << "' doesn't existed!" << std::endl;
    }
    uniform_lcoation_cache_[name] = location;
    return location;
